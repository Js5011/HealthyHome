...
def emergency(text, ai_response):
    alert(text, ai_response)
        ...
        if ser.in_waiting > 0:
            ...
            try:
                ...
                if "high risk" in ai_response:
                    ai_response = ask_ai(f"Temperature: {temp}C, Humidity: {hum}%, high risk conditoins are detected, explain")
                    emergency('DANGEROUS CONDITIONS DETECTED', ai_response)

                now = datetime.now()
                if now.hour == 9 and (last_daily_report_day != now.date()):
               # last_daily_report_day = now.date()

                    daily = ask_ai(
                    f"Daily environmental report at 9AM. Temperature: {temp}C, Humidity: {hum}%")
                    alert("DAILY REPORT: " + daily)
